...
List getJervisMetaData(String project, String JERVIS_BRANCH) {
    ...
    boolean jervisFileExists = false
    for(String file: folder_listing) {
        if(file.startsWith(".jervis") && file.endsWith(".yml")) {
            String component_name = file.substring(7, file.lastIndexOf(".yml"))
            if(! component_name) {
                component_name = 'main'
            }
            jervis_yamls_map.put(component_name, git_service.getFile(project, file, JERVIS_BRANCH))
    ...
    if(! jervisFileExists) {
        if('.jervis-default.yml' in aurea_central_listing) {
            ...
            jervis_yamls_map.put('main', git_service.getFile(project, '/aurea-central-jervis/.jervis-default.yml', JERVIS_BRANCH))
        }
        else {
            throw new FileNotFoundException('Cannot find .jervis.yml nor .travis.yml')
        }
    }
    [jervis_yamls_map, folder_listing]
}
...
def call() {
    ...
    jervis_yamls_map = jervis_metadata[0]
    folder_listing = jervis_metadata[1]

    //Remove components which are not relevant for current branch
    List components_to_be_removed = new ArrayList()
    for(Map.Entry jervis_yaml_entry: jervis_yamls_map.entrySet()) {
        def generator = new lifecycleGenerator()
        platforms_json = loadCustomResource 'platforms.json'
        generator.loadPlatformsString(platforms_json)
        generator.preloadYamlString(jervis_yaml_entry.value)
        ...
        generator.loadYamlString(jervis_yaml_entry.value)
        generator.folder_listing = folder_listing
        //check for branch filters
        if(BRANCH_NAME && ! generator.isGenerateBranch(BRANCH_NAME)) {
            components_to_be_removed.add(jervis_yaml_entry.key)
        }
    }
    for(String component : components_to_be_removed) {
        jervis_yamls_map.remove(component)
    }

    Map jervis_tasks = [failFast: true]

    jervis_yamls_map.keySet().each{
        component_name ->
            jervis_tasks[component_name] = {
                node('jervis_generator'){
                    stage("Forking pipeline for components") {
                        buildViaJervisForComponent(jervis_yamls_map[component_name],folder_listing,component_name)
                    }
                }
            }
    }
...
def buildViaJervisForComponent(String jervis_yaml, List folder_listing, String component_name) {
    ...
    stage("Process Jervis YAML for component ${component_name}") {
    ...
    List trigger_cron_branches = generator.jervis_yaml['trigger_cron_branch']
    String trigger_cron_expression = generator.getObjectValue(generator.jervis_yaml, 'trigger_cron_expression', '')
    if(trigger_cron_branches && trigger_cron_expression) {
        if(BRANCH_NAME in trigger_cron_branches) {
            properties([pipelineTriggers([[$class: "TimerTrigger", spec: "${trigger_cron_expression}"]])])
        }
    }
